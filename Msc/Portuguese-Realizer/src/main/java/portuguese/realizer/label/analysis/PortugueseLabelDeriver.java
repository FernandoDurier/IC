...
public class PortugueseLabelDeriver implements ILabelDeriver
{
	private ILabelHelper lHelper;
	
	public PortugueseLabelDeriver(ILabelHelper labelHelper)
	{
		lHelper = labelHelper;
	}
	...
	public void deriveFromVOS(String label, String[] labelSplit, ILabelProperties props) throws LabelException 
	{
		...
		if (label.contains(" e ") || label.contains("/"))
		{
			for (int i = 1; i < labelSplit.length; i++) 
			{
				if (labelSplit[i].equals("e") || labelSplit[i].equals("/")) 
				{
		...
		{
			props.setAction(labelSplit[0] + " e " + labelSplit[props.getIndexConjunctionSplit()+1]);
