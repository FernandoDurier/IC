...
public class PortugueseSurfaceRealizer extends SurfaceRealizer
{
	/**
	...
	public PortugueseSurfaceRealizer(ILabelHelper labelHelper, INaturalLanguageProcessor languageProcessor) 
	{
	...
	private String realizeFragment(AbstractFragment frag, boolean isCondition, boolean verbFirst) 
	{
		...
		bo = bo.trim();
		if(bo.contains("os seguintes caminhos"))
			bo = "Em seguida, os seguintes caminhos ";
		...
		if(verb.equalsIgnoreCase("executado"))
			verb += "s";
		...
		{	
			...
			{
				if(verbSplitted.length < 1)
					verb = " tenha " + VerbManager.getInstance().getParticipleForm(frag.getAction());
			}
