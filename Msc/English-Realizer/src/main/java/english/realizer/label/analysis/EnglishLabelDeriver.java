package english.realizer.label.analysis;

import general.language.common.label.analysis.Activity;
...
public class EnglishLabelDeriver implements ILabelDeriver 
{
	...
	public void processLabel(Activity activity, String labelStyle) {
		...
		try {
			...
			lHelper.checkForPrepositions(label, labelSplit, props);
	 ...
	 * @param props
	 */
	private void deriveFromPluralNounLabel(String label, String[] labelSplit, ILabelProperties props) {
		props.setAction(labelSplit[0]);
		for (int j = 1; j <= labelSplit.length - 1; j++) {
			props.setBusinessObject(props.getBusinessObject() + " "
					+ labelSplit[j]);
	 ...
	 */
	private void deriveFromIrregular(String label, String[] labelSplit, Activity activity, EnglishLabelProperties props) throws LabelException {
		try {
			...
			e.printStackTrace();
			throw new LabelException(e.getMessage());
		}
	}
	 ...
	 */
	private void deriveFromNounPhraseADV(String label, String[] labelSplit, ILabelProperties props) throws LabelException {
		String adv;
		try {
			adv = lHelper.getAdverb(wordnet.lookupAllIndexWords(labelSplit[0]).getIndexWord(POS.ADJECTIVE).getLemma());
		} catch (JWNLException e) {
			e.printStackTrace();
			throw new LabelException(e.getMessage());
		}
	 ...
	 * @param props property object
	 */
	private void deriveFromNounPhrase(String label, String[] labelSplit, ILabelProperties props) {
		props.setAction(labelSplit[labelSplit.length-1]);
	 ...
	 * @throws JWNLException 
	 */
	private void deriveFromNounPhraseWithPrepPhraseADV(String label, String[] labelSplit, ILabelProperties props) throws LabelException {
		String adv;
		try {
			adv = lHelper.getAdverb(wordnet.lookupAllIndexWords(labelSplit[0]).getIndexWord(POS.ADJECTIVE).getLemma());
		} catch (JWNLException e) {
	 ...
	 * @param props property object
	 */
	private void deriveFromNounPhraseWithPrepPhrase(String label, String[] labelSplit, ILabelProperties props) throws LabelException{
		props.setAction(getWordBeforeIndex(label, props.getIndexPrep()));
		for (int j=0; j<=props.getIndexPrepSplit()-2; j++) {
			props.setBusinessObject(props.getBusinessObject() + " " + labelSplit[j]);
		}
	 ...
	 * @throws JWNLException 
	 */
	private void deriveFromPrepositionOfADV(String label, String[] labelSplit, ILabelProperties props) throws LabelException 
	{
		try
		{
		...
		{
			throw new LabelException(jwnlException.getMessage());
		}
	}
	 ...
	 * @throws JWNLException
	 */
	private void deriveFromGerundStyle(String label, String[] labelSplit, ILabelProperties props) throws LabelException {
			if (labelSplit.length > 1) {
				if (checkForPhrasalVerb(lHelper.getInfinitiveOfAction(labelSplit[0]) + " " + labelSplit[1])) {
					props.setHasPhrasalVerb(true);
				}
	 ...
	 * @param props property object
	 */
	private void deriveFromPrepositionOf(String label, String[] labelSplit, ILabelProperties props) {
		props.setAction(getWordBeforeIndex(label, props.getIndexPrep()));
		int temp = -1;
		for (int j=props.getIndexOf()+1; j<=labelSplit.length-1; j++) {
			if (props.getPrepositions().contains(labelSplit[j]) == true) {
	...
	public void deriveFromDES(String label, String[] labelSplit, Activity activity, ILabelProperties props) {
	...
	private boolean checkForPhrasalVerb(String word) throws LabelException {
		IndexWord indexWord;
		try {
			indexWord = wordnet.getIndexWord(POS.VERB,word);
		} catch (JWNLException e) {
			e.printStackTrace();
			throw new LabelException(e.getMessage());
		}
